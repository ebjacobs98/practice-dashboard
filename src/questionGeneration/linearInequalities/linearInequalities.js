const getLinearInequalitiesQuestion = () => {
  ...
  const isQuestionGreaterThan = Math.random() >= 0.5;
  const questionSymbol = isQuestionGreaterThan ? ">" : "<";
  ...
  const shouldSwitchSign = combinedCoefficient < 0;
  const answerSign = shouldSwitchSign
    ? !isQuestionGreaterThan
    : isQuestionGreaterThan;
  ...
  if (isLeftValueFirst) {
    if (isLeftCoefficientPositive) {
      equation = `${mathLeftValue} + ${leftCoefficient}x ${questionSymbol} `;
    } else {
      equation = `${mathLeftValue} - ${leftCoefficient}x ${questionSymbol} `;
    }
  } else {
    if (isLeftValuePositive) {
      equation = `${mathLeftCoefficient}x + ${leftValue} ${questionSymbol} `;
    } else {
      equation = `${mathLeftCoefficient}x - ${leftValue} ${questionSymbol} `;
    }
  }
  ...
  return {
    question: expression,
    answers: [
      { answer: answerSign ? answer : "", answerLabel: "x > ", error: 0.01 },
      { answer: answerSign ? "" : answer, answerLabel: "x < ", error: 0.01 },
    ],
  };
};

export default getLinearInequalitiesQuestion;
